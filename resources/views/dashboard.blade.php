...
@php
    ...
    $canSeeLedger = $currentUser->hasModuleAccess(\App\Models\User::MODULE_LEDGER);
    $canSeeDocuments = $currentUser->hasModuleAccess(\App\Models\User::MODULE_DOCUMENTS);
    $canReviewMonitoring = $currentUser->hasModuleAccess(\App\Models\User::MODULE_MONITORING_REVIEW);
    $canSubmitMonitoring = $currentUser->hasModuleAccess(\App\Models\User::MODULE_MONITORING_SUBMIT);
    $canSeeInventory = $currentUser->hasModuleAccess(\App\Models\User::MODULE_INVENTORY);
    $canRequestMaterials = $currentUser->hasModuleAccess(\App\Models\User::MODULE_MATERIAL_REQUESTS);
    $canApproveMaterials = $currentUser->hasModuleAccess(\App\Models\User::MODULE_MATERIAL_APPROVALS);
    $canManageProjectSettings = $currentUser->canManageProjectsSettings();
    $canManageUsers = $currentUser->canManageUsers();
    $cardCount = collect([
        $canSeeLedger,
        $canSeeDocuments,
        $canReviewMonitoring || $canSubmitMonitoring,
        $canRequestMaterials,
        $canSeeInventory,
        $canApproveMaterials,
...
@section('content')
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .ledger-icon { background: rgba(37,99,235,0.1); color: var(--ledger); }
        .document-icon { background: rgba(15,159,143,0.1); color: var(--document); }
        .project-icon { background: rgba(8,145,178,0.1); color: var(--project); }
        .inventory-icon { background: rgba(8,145,178,0.1); color: var(--inventory); }
        .users-icon { background: rgba(13,148,136,0.1); color: var(--users); }
        .settings-icon { background: rgba(29,78,216,0.1); color: var(--settings); }
        ...
        .settings-text { color: var(--settings); }
        ...
        .logout-btn:hover { background: #ecfdf8; border-color: #99dacf; color: #0f766e; }
        ...
        <div class="dashboard-container">
            ...
            @if($cardCount === 0)
                <div class="empty-state">
                    No modules are assigned to this account yet. Ask an admin to update your access in User Management.
                </div>
            @endif
            ...
            <div class="cards-grid">
                @if($canSeeLedger)
                    <a href="{{ route('projects.index') }}" class="choice-card" style="animation-delay: 0.1s;">
                        ...
                        <p class="card-description">Manage project finances, track budgets, monitor expenses, and export financial reports.</p>
                ...
                @if($canReviewMonitoring || $canSubmitMonitoring)
                    <a href="{{ $canReviewMonitoring ? route('monitoring.index') : route('monitoring.submit') }}" class="choice-card" style="animation-delay: 0.3s;">
                        ...
                        <p class="card-description">{{ $canReviewMonitoring ? 'Review reports, approve progress, and monitor completion metrics.' : 'Submit accomplishments, upload photos, and review your approval status.' }}</p>
                        <div class="card-footer">
                ...
                @if($canSeeInventory)
                    <a href="{{ route('inventory.index') }}" class="choice-card" style="animation-delay: 0.5s;">
                        ...
                        <p class="card-description">Manage materials, equipment, and supplies. Assign stock to projects and export reports.</p>
                        <div class="card-footer">
                ...
                @if($canApproveMaterials)
                    <a href="{{ route('material-requests.index') }}" class="choice-card" style="animation-delay: 0.6s;">
                        ...
                        <p class="card-description">Review requests, reserve budget, issue stock, and handle procurement decisions.</p>
                        <div class="card-footer">
                ...
                @if($canManageProjectSettings)
                    <a href="{{ route('settings.projects.index') }}" class="choice-card" style="animation-delay: 0.7s;">
                        ...
                        <p class="card-description">Create, update, and maintain project master records and setup details.</p>
                        <div class="card-footer">
                ...
                @if($canManageUsers)
                    <a href="{{ route('settings.users.index') }}" class="choice-card" style="animation-delay: 0.8s;">
                        ...
                        <p class="card-description">Add user accounts, assign module ownership, and control operational access per account.</p>
                        <div class="card-footer">
            ...
            @if($canSeeLedger && isset($projects) && $projects->count() > 0)
